#!/bin/bash
# Create enhanced Gantt Chart template with fixed navigation
...
echo -e "${BLUE}Creating enhanced Gantt Chart template...${NC}"
cat > frontend/templates/feature/gantt.html << 'EOF'
<!DOCTYPE html>
<html>
<head>
...
EOF
echo -e "${GREEN}✓ Created enhanced Gantt Chart template${NC}"
echo ""

# Create a shell script to apply all the patches
echo -e "${BLUE}Creating master patch script...${NC}"
cat > apply_all_patches.sh << 'EOF'
#!/bin/bash
...
echo -e "${GREEN}✓ Created master patch script${NC}"
echo ""

echo -e "${GREEN}All patch files created successfully!${NC}"
echo -e "${BLUE}To apply the patches, run:${NC}"
echo -e "${YELLOW}   ./apply_all_patches.sh${NC}"
