...
cat << 'FIX_TEST_PAGE_SCRIPT' > fix_test_page.sh
#!/bin/bash
set -e

echo "🔧 Fixing test page route and creating test page..."
...
# Create a simple route patch file
cat << 'EOF' > app_route_patch.py
...
# Create a simple script to update the app.py file
...
chmod +x update_app.py

echo "Running patch to add test route to app.py..."
python3 update_app.py

echo "✅ Test page setup is complete!"
echo "🔹 Restart your Flask server with: python3 backend/app.py"
echo "🔹 Then visit: http://127.0.0.1:5000/test/gantt"
FIX_TEST_PAGE_SCRIPT

chmod +x fix_test_page.sh
echo "✅ Created fix script. Run it with: ./fix_test_page.sh"
