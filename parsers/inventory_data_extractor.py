...
def normalize_filename(filename: str) -> str:
    """
    Normalize filename to a base name for duplication detection
    
    Args:
    filename (str): Original filename
    
    Returns:
    str: Normalized base filename for comparison
    """
    # Remove file extension
    base_name = os.path.splitext(filename)[0]
    
    # Remove common suffixes that indicate file format
    format_indicators = ['.xlsx', '.xls', '.ods', '.csv', ' - Jumpers']
...
def get_processed_files(output_dir: str) -> Set[str]:
    """
    Get a set of already processed base filenames
    
    Args:
    output_dir (str): Directory with output files
    
    Returns:
    Set[str]: Set of normalized base filenames
    """
    processed = set()
    if os.path.exists(output_dir):
        for file in os.listdir(output_dir):
            if file.endswith('.json'):
                processed.add(normalize_filename(file))
    return processed

def process_files(input_dir: str, output_dir: str, logger: logging.Logger, 
                  skip_duplicates: bool = True) -> Dict[str, List[str]]:
    """
    ...
    logger (logging.Logger): Logger instance
    skip_duplicates (bool): Whether to skip duplicate files
    ...
    # Get already processed files if skipping duplicates
    processed_base_names = get_processed_files(output_dir) if skip_duplicates else set()
    
    # Priority order for file formats (we prefer certain formats over others)
    format_priority = {'.xlsx': 1, '.xls': 2, '.ods': 3, '.csv': 4, '.pdf': 5}
    
    # Supported file extensions
    supported_extensions = ['.pdf', '.xlsx', '.xls', '.ods', '.csv']

    # First, collect all files and sort by format priority
    inventory_files = []
    for ext in supported_extensions:
        inventory_files.extend(list(Path(input_dir).glob(f'*{ext}')))
    
    # Sort files by normalized base name and format priority
    inventory_files.sort(key=lambda x: (
        normalize_filename(x.name), 
        format_priority.get(os.path.splitext(x.name)[1].lower(), 99)
    ))
    
    logger.info(f"Found {len(inventory_files)} supported files in {input_dir}")
    
    # Track normalized names we've already processed in this run
    ...
    for input_path in inventory_files:
        ...
        normalized_name = normalize_filename(inventory_file)
        
        # Skip if we've already processed this base file name
        if normalized_name in processed_base_names or normalized_name in processed_in_current_run:
            logger.info(f"Skipping duplicate file: {inventory_file} (already processed)")
            processed_files['skipped'].append(base_filename)
            continue
        ...
        try:
            ...
            if inventory_data['wardrobe_items']:
                ...
                processed_in_current_run.add(normalized_name)
            else:
...
def main():
    ...
    parser.add_argument(
        '--force', 
        action='store_true', 
        help='Process all files even if duplicates exist'
    )
    parser.add_argument(
        '--clean', 
        action='store_true', 
    ...
    for input_dir in input_dirs:
        ...
        logger.info(f"  - Skipped duplicates: {len(processed['skipped'])}")
